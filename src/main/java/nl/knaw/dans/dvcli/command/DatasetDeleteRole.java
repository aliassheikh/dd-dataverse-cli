...
@Command(name = "remove-role", mixinStandardHelpOptions = true, description = "remove role assignment from specified dataset(s)")
public class DatasetDeleteRole extends AbstractAssignmentRole {
    @ParentCommand
    ...
    private static class RoleAssignmentAction implements ThrowingFunction<RoleAssignmentParams, String, Exception> {
        ...
        public String apply(RoleAssignmentParams roleAssignmentParams) throws IOException, DataverseException {
            if (roleAssignmentParams.roleAssignment().isPresent()) {
                // TODO var r = roleAssignmentParams.pid().deleteRole(roleAssignmentParams.roleAssignment.get());
                // TODO return r.getEnvelopeAsString();
                // TODO in DatasetApi: https://guides.dataverse.org/en/latest/api/native-api.html#delete-role-assignment-from-a-dataset
                return "Couldn't process 'Remove Role Assignment'";
            }
